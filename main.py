...
C_RESULT    = (0.20, 0.75, 0.55, 1)  # teal-green for Results button
C_GOLD      = (1.00, 0.82, 0.20, 1)  # gold for total highlight
...
class SettingsPopup(Popup):
    def __init__(self, app_ref, **kwargs):
        ...
        root.add_widget(Label(
            text="Cone Calculator Settings",
            color=C_ACCENT, font_size=dp(16), bold=True,
        ...
        close_btn = make_button("Close", C_PRIMARY)
        close_btn.bind(on_press=self.dismiss)
        root.add_widget(close_btn)

        root.add_widget(Label(
            text="✦  Created by Ken  ✦",
            color=(*C_PRIMARY[:3], 0.45),
...
class DatePickerPopup(Popup):
    def __init__(self, date_list, on_date_selected, title_text="Select Date", **kwargs):
        ...
        if not date_list:
            ...
            for ds in date_list:
                btn = make_button(f"📅  {ds}", C_SURFACE, C_TEXT)
...
class RecordListPopup(Popup):
    def __init__(self, date_str, records, indices,
        ...
        root.add_widget(Label(
            text="Tap a record to edit  •  🗑 to delete",
